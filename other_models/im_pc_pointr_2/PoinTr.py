...
class PoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.base_model = PCTransformer(in_chans = 3, embed_dim_img = self.trans_dim_img, embed_dim_pc = self.trans_dim_pc, depth = [6, 8], drop_rate = 0., num_query_img = self.num_query_img, num_query_pc = self.num_query_pc, knn_layer = self.knn_layer)
        ...
        self.foldingnet = Fold(self.trans_dim_pc, step = self.fold_step, hidden_dim = 256)  # rebuild a cluster point todo
    ...
    def postprocess(self, q, coarse_point_cloud):
        querries_all = coarse_point_cloud

        B, M, C = q.shape

        global_feature = self.increase_dim_pc(q.transpose(1, 2)).transpose(1, 2)  # B M 1024
        ...
        rebuild_feature = self.reduce_map_pc(rebuild_feature.reshape(B * M, -1))  # BM C

        # NOTE: foldingNet
        relative_xyz = self.foldingnet(rebuild_feature).reshape(B, M, 3, -1)  # B M 3 S

        rebuild_points_0 = (relative_xyz + coarse_point_cloud.unsqueeze(-1)).transpose(2, 3).reshape(B, -1, 3)  # B N 3

        return [querries_all, rebuild_points_0]

    def forward(self, imgs_feat, pc):
        q_img, q_pc, coarse_point_cloud_img, coarse_point_cloud_pc = self.base_model(imgs_feat, pc) # B M C and B M 3
        ret_img = self.postprocess(q_img,coarse_point_cloud_img)
        ret_pc = self.postprocess(q_pc, coarse_point_cloud_pc)
        B, M, C = q_pc.shape
        rebuild_points_1 = torch.cat([ret_pc[1], fps(pc, self.num_pred - M * self.fold_step ** 2)],
                                     dim=1).contiguous()
        ret_pc.append(rebuild_points_1)


        return ret_img, ret_pc
