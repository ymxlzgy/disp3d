...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim_img=768, embed_dim_pc=768, depth=[6, 6], num_heads=8, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.coarse_pred_pc_1 = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query_pc)
        )
        self.coarse_pred_pc_2 = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query_pc)
        )
        self.mlp_query_1 = nn.Sequential(
        ...
        self.mlp_query_2 = nn.Sequential(
            nn.Conv1d(1024 + 3, 1024, 1),
            # nn.BatchNorm1d(1024),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(1024, 1024, 1),
    ...
    def shared_decoder(self, query_f, global_f, self_knn_index, cross_knn_index):
        for i, blk in enumerate(self.decoder):
            if i < self.knn_layer:
                query_f = blk(query_f, global_f, self_knn_index, cross_knn_index)  # B M C
            else:
                query_f = blk(query_f, global_f)
        return query_f
    ...
    def forward(self, imgs_feat, pc_feat):
        ...
        global_feature_img = torch.max(global_feature_img, dim=-1)[0]  # B 1024
        coarse_point_cloud_img = self.coarse_pred_pc_1(global_feature_img).reshape(bs, -1, 3)  # B M C(3)
        new_knn_index_img = get_knn_index(coarse_point_cloud_img.transpose(1, 2).contiguous())
        cross_knn_index_img = new_knn_index_img
        query_feature_img = torch.cat([
            global_feature_img.unsqueeze(1).expand(-1, self.num_query_pc, -1),
            coarse_point_cloud_img], dim=-1)  # B M C+3
        q_img = self.mlp_query_1(query_feature_img.transpose(1, 2)).transpose(1, 2)  # B M C
        # decoder
        q_img = self.shared_decoder(q_img, x, new_knn_index_img, cross_knn_index_img)


        global_feature_pc = self.increase_dim_pc(y.transpose(1, 2))  # B 1024 N2
        global_feature_pc = torch.max(global_feature_pc, dim=-1)[0]  # B 1024
        coarse_point_cloud_pc = self.coarse_pred_pc_2(global_feature_pc).reshape(bs, -1, 3)  # B M C(3)
        new_knn_index_pc = get_knn_index(coarse_point_cloud_pc.transpose(1, 2).contiguous())
        cross_knn_index_pc = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud_pc.transpose(1, 2).contiguous())
        query_feature_pc = torch.cat([
            global_feature_pc.unsqueeze(1).expand(-1, self.num_query_pc, -1),
            coarse_point_cloud_pc], dim=-1)  # B M C+3
        q_pc = self.mlp_query_2(query_feature_pc.transpose(1, 2)).transpose(1, 2)  # B M C
        # decoder
        q_pc = self.shared_decoder(q_pc, y, new_knn_index_pc, cross_knn_index_pc)

        return q_img, q_pc, coarse_point_cloud_img, coarse_point_cloud_pc
