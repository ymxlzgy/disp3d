...
from .Transformer import PCTransformer_encoder
from .build import MODELS
import other_models.im_pc_disp3d_trans.encode as disp3d_encoder
import other_models.im_pc_disp3d_trans.decode as disp3d_decoder
...
class Disp_PoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        # self.fold_step = int(pow(self.num_pred//self.num_query_pc, 0.5) + 0.5) #todo
        ...
        self.coarse_pred_pc = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * config.num_query_pc)
        )
        self.disp_enc = disp3d_encoder.Encoder(support_num=10, neighbor_num=20,config=config)
        self.disp_dec = disp3d_decoder.Decoder(
            features=[64, 3],
            degrees=[1],
            support=10,
            neighbor_num=20,
            root_num=1,config=config)
        self.reduce_dim = nn.Sequential(
            nn.Conv1d(self.trans_dim_pc+3, 3, 1),
            nn.BatchNorm1d(3),
        ...
        self.mlp_query = nn.Sequential(
            ...
            # nn.BatchNorm1d(1024),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(1024, self.trans_dim_pc, 1)
        )
        self.reduce_map_img = nn.Linear(self.trans_dim_img + 1027, self.trans_dim_img)
        self.reduce_map_pc = nn.Linear(self.trans_dim_pc + 1027, self.trans_dim_pc)
    ...
    def forward(self, imgs_feat, part):
        assert imgs_feat.size(0) == part.size(0)
        bs = imgs_feat.size(0)
        coor, single_feat_global, global_f = self.disp_enc(imgs_feat,part)
        coarse_point_cloud = self.coarse_pred_pc(global_f).reshape(bs, -1, 3)  # B M C(3)
        new_knn_index = disp3d_encoder.get_knn_index(coarse_point_cloud.transpose(1, 2).contiguous())
        cross_knn_index = disp3d_encoder.get_knn_index(coor_k=coor, coor_q=coarse_point_cloud.transpose(1, 2).contiguous())

        query_feature = torch.cat([
            global_f.unsqueeze(1).expand(-1, self.num_query_pc, -1),
            coarse_point_cloud], dim=-1)  # B M C+3
        q = self.mlp_query(query_feature.transpose(1, 2)).transpose(1, 2)  # B M C

        rebuild_q = torch.cat([q, coarse_point_cloud], dim=-1)

        rebuild_coarse_point_cloud = self.reduce_dim(rebuild_q.transpose(1,2)).transpose(1,2) # B M 3
        point_cloud_1, point_cloud_2 = self.disp_dec(rebuild_coarse_point_cloud)





        return [coarse_point_cloud, point_cloud_1, point_cloud_2]
